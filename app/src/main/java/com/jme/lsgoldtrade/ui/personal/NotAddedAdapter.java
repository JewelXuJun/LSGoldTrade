...
public class NotAddedAdapter extends BaseQuickAdapter<NavigatorVo.NavigatorVoBean, BaseViewHolder> {

    private boolean bEditFlag = false;

    public NotAddedAdapter(int resId, List<NavigatorVo.NavigatorVoBean> data) {
        super(resId, data);
    }
    ...
    protected void convert(BaseViewHolder helper, NavigatorVo.NavigatorVoBean item) {
        ...
        helper.setText(R.id.tv_tab_name, item.getName())
                .setGone(R.id.layout_add, bEditFlag)
                .addOnClickListener(R.id.layout_add);
    }
}
