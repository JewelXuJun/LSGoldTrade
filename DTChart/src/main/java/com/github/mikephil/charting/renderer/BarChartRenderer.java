...
// Modify by yanmin: 把继承DataRenderer改成继承LineScatterCandleRadarRenderer，支持HighLight以画线形式来表现
public class BarChartRenderer extends LineScatterCandleRadarRenderer {
    ...
    public BarChartRenderer(BarDataProvider chart, ChartAnimator animator,
        ...
        //Modify by yanmin: 把HighLight由高亮Bar改成画线形式
...
//        mHighlightPaint.setAlpha(120);
    ...
    protected void drawDataSet(Canvas c, BarDataSet dataSet, int index) {
        ...
        //Modify: 计算出bar的宽度，set画笔的宽度，然后把DrawRect改成DrawLine来实现 By yanmin
        float lineWidth = buffer.buffer[2] - buffer.buffer[0];
        if (lineWidth < 1f && lineWidth != 0f)
            lineWidth = 1f;
        mRenderPaint.setStrokeWidth(lineWidth);

        // if multiple colors
//        if (dataSet.getColors().size() == 2) {
//
...
//                }
//
//                // Set the color for the currently drawn value. According to the distance from the bottom of the view and the height ,
//                // set different colors.
//
//                //Modify: 值大于0和小于0的，用不同的颜色表示 by yanmin
...
//
//                //Modify: 把DrawRect改成DrawLine来实现 By yanmin
//                float mid = lineWidth / 2 + buffer.buffer[j];
//                if (lineWidth != 0f) {
//                    c.drawLine(mid, buffer.buffer[j + 1], mid,
...
//        } else {
        ...
        for (int j = 0; j < buffer.size(); j += 4) {
            ...
            if (dataSet.getColors().size() == (buffer.size() / 4)) {
                mRenderPaint.setColor(dataSet.getColor(j / 4));
            }
...
//        }
    ...
    //@Override
    public void drawHighlighted2(Canvas c, Highlight[] indices) {
...
//                pts[0] = indices[i].getX();
            ...
            //Add By yanmin: 显示当前HighLight对应的Left-Y值
            if (set.isHorizontalHighlightIndicatorEnabled()
                    && leftYAxis.isDrawLabelsEnabled()) {
