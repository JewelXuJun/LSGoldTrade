...
public class YAxisRenderer extends AxisRenderer {
    ...
    protected void computeAxisValues(float min, float max) {
        ...
        if (mYAxis.isForceLabelsEnabled()) {
            ...
            if (mYAxis.isMeristic() && labelCount % 2 == 1) {
                //Modify by yanmin:当有设置中间值，并且labelCount是奇数时，以中间值为基线，上下减step，保证中间的线一定等于中间值
                int mid = labelCount / 2;
                float v = mYAxis.getMedian();
                for (int i = mid; i < labelCount; i++) {
                    mYAxis.mEntries[i] = v;
                    v += step;
                }
                v = mYAxis.getMedian();
    ...
    protected void drawYLabels(Canvas c, float fixedPosition, float[] positions, float offset) {
        //Modify by yanmin: 当isDrawTopYLabelEntryEnabled为true，最顶部的Lable往下移动位置保证可以正常显示
        float y;
        float height = Utils.calcTextHeight(mAxisLabelPaint, "A") - offset;
        ...
        for (int i = 0; i < mYAxis.mEntryCount; i++) {
            ...
            //Modify by yanmin: 判断所画Label使用什么颜色（涨，跌，平）
            int compare = mYAxis.isLargerThanMedian(i);
            int primary = mAxisLabelPaint.getColor();
            if (compare == 1)
                mAxisLabelPaint.setColor(mYAxis.getIncreasingColor());
            else if (compare == -1)
                mAxisLabelPaint.setColor(mYAxis.getDecreasingColor());

            c.drawText(text, fixedPosition, y, mAxisLabelPaint);
            mAxisLabelPaint.setColor(primary);
        }
    }
...
//        mGridPaint.setPathEffect(mYAxis.getGridDashPathEffect());
        ...
        // Modify by yanmin:不画最上最下的两条线，防止跟边框重合
        for (int i = 1; i < mYAxis.mEntryCount - 1; i++) {

            // Modify by yanmin:如果有中间值（昨收价），则中间值对应的线用实线画
            if (mYAxis.isMeristic() & mYAxis.getMedian() == mYAxis.mEntries[i]) {
    ...
    public void renderNodataGridLines(Canvas c) {
        ...
        Path gridLinePath = new Path();
        // Modify by yanmin:显示跟有数据时一样个数的数值线
        int count = mYAxis.getLabelCount() - 1;
        float itemHeight = mViewPortHandler.getContentRect().height() / count;//mViewPortHandler.getChartHeight()

        // draw the horizontal grid
        for (int i = 1; i < count; i++) {

            float y = mViewPortHandler.offsetTop() + itemHeight * i;
