...
public class ChartHighlighter<T extends BarLineScatterCandleBubbleDataProvider> {
	...
	public Highlight getHighlight(float x, float y) {
		...
		// Modify by yanmin: 返回选中的xIndex的SelectionDetail对象
//		int dataSetIndex = getDataSetIndex(xIndex, x, y);
//		if (dataSetIndex == -Integer.MAX_VALUE)
//			return null;
		SelectionDetail selectionDetail = getSelectionDetail(xIndex, x, y);
		if (selectionDetail == null)
			return null;

		return new Highlight(selectionDetail.xIndex, selectionDetail.dataSetIndex, x, y);
	}
	...
	// Modify By yanmin: 返回Closest SelectionDetail 对象，功能同上一个函数
	protected SelectionDetail getSelectionDetail(int xIndex, float x, float y) {
		...
		SelectionDetail selectionDetail = Utils.getClosestSelectionDetail(valsAtIndex, y, axis);
