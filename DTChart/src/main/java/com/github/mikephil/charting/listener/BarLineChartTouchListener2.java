 ...
 */
public class BarLineChartTouchListener2 extends ChartTouchListener<BarLineChartBase<? extends BarLineScatterCandleBubbleData<? extends BarLineScatterCandleBubbleDataSet<? extends Entry>>>> {
    ...
    //Add: 添加对副Chart的操作。by Yanmin
    private Matrix mMatrixSecond = new Matrix();
    private boolean hasSecondChart = false;
    ...
    //Add: 添加对副Chart的操作。 By Yanmin
    private Matrix mSavedMatrixSecond = new Matrix();
    ...
    public BarLineChartTouchListener2(BarLineChartBase<? extends BarLineScatterCandleBubbleData<? extends BarLineScatterCandleBubbleDataSet<? extends Entry>>> chart, Matrix touchMatrix) {
        super(chart);
        this.mMatrix = touchMatrix;

    }

    //Add: 设置副Chart。 By Yanmin
    public void setSecondChart(BarLineChartBase<? extends BarLineScatterCandleBubbleData<? extends BarLineScatterCandleBubbleDataSet<? extends Entry>>> chartSecond) {
        super.setSecondChart(chartSecond);
        this.mMatrixSecond = chartSecond.getViewPortHandler().getMatrixTouch();
        hasSecondChart = true;
    }
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        //Bug: 修复当手指Move时，会出发LongPress事件，是因为mTouchMode不为None时，不再调用mGestureDetector的onTouchEvent，从而
        //造成LongPress事件没有及时Remove，故把改If条件去除，有可能会对其它造成影响，By yanmin
        //if (mTouchMode == NONE) {
        ...
        switch (event.getAction() & MotionEvent.ACTION_MASK) {
                ...
                if (event.getPointerCount() >= 2) {
                    ...
                    if (mSavedDist > 10f) {
                        ...
                        if (mChart.isPinchZoomEnabled()) {
                            ...
                            //Modify：让两个手指Zoom不区分Y轴的Zoom，同时也就禁用了YZoom。 by yanmin
                    ...
                    // Modify By yanmin:双指缩放的中心恒定设在控件的最右端
                    mTouchPointCenter.x = v.getWidth();
                }
                break;
            case MotionEvent.ACTION_MOVE:
                ...
                if (mTouchMode == DRAG) {
                    ...
                    if (mChart.isHighlightPerDragEnabled() && bHightLightShow) {
                        performHighlightDrag(event);
                        performHighlightDragSecond(event);
                    }
//                        }
                    //}
            ...
            case MotionEvent.ACTION_UP:
                ...
                //Modify: 当ActionUp时，取消显示HighLight By yanmin
                if (mChart.isHighlightPerDragEnabled() && bHightLightShow) {
                    mChart.highlightTouch(null);
                    if (hasSecondChart) {
                        mChartSecond.highlightTouch(null);
                    }
                    bHightLightShow = false;
                }
        ...
        if (hasSecondChart) {
            mMatrixSecond = mChartSecond.getViewPortHandler().refresh(mMatrixSecond, mChartSecond, true);
        }
    ...
    private void saveTouchStart(MotionEvent event) {
        ...
        if (hasSecondChart) {
            mSavedMatrixSecond.set(mMatrixSecond);
        }
    ...
    private void performDrag(MotionEvent event) {
        ...
        if (hasSecondChart) {
            mMatrixSecond.set(mSavedMatrixSecond);
        }
        ...
        if (hasSecondChart) {
            mMatrixSecond.postTranslate(dX, dY);
        }
        if (l != null)
            ...
            if (totalDist > 10f) {
                ...
                if (mTouchMode == PINCH_ZOOM) {
                    ...
                    if (mChart.isScaleYEnabled() || canZoomMoreX) {
                        ...
                        if (hasSecondChart) {
                            mMatrixSecond.set(mSavedMatrixSecond);
                            mMatrixSecond.postScale(scaleX, scaleY, t.x, t.y);
                        }
                    ...
                    if (canZoomMoreX) {
                        ...
                        if (hasSecondChart) {
                            mMatrixSecond.set(mSavedMatrixSecond);
                            mMatrixSecond.postScale(scaleX, 1f, t.x, t.y);
                        }
                        if (l != null)
                    ...
                    if (hasSecondChart) {
                        mMatrixSecond.set(mSavedMatrixSecond);
                        mMatrixSecond.postScale(1f, scaleY, t.x, t.y);
                    }
    ...
    private void performHighlightDragSecond(MotionEvent e) {
        if (hasSecondChart) {
            Highlight h = mChartSecond.getHighlightByTouchPoint(e.getX(), e.getY());

            if (h != null && !h.equalTo(mLastHighlightedSecond)) {
                mLastHighlightedSecond = h;
                mChartSecond.highlightTouch(h);
            }
        }
    }
    ...
    private boolean bHightLightShow = false;

    @Override
    public void onLongPress(MotionEvent e) {
        //Modify by hualei:chart长按时，阻止外层onTouch事件的调用
        ...
        //Modify: 改为当LongPress时，显示HighLight（ActionUp时取消） By yanmin
        if (mChart.isHighlightPerTapEnabled() && mTouchMode == NONE) {
            Highlight h = mChart.getHighlightByTouchPoint(e.getX(), e.getY());
            performHighlight(h, e);

            if (hasSecondChart) {
                Highlight h2 = mChartSecond.getHighlightByTouchPoint(e.getX(), e.getY());
                performHighlightSecond(h2, e);
            }

            bHightLightShow = true;
    ...
    public boolean onSingleTapUp(MotionEvent e) {
        ...
        //Modify: 删除单击显示HighLight，改成LongPress时显示 By yanmin
