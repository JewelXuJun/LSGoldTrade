...
import com.github.mikephil.charting.listener.BarLineChartTouchListener2;
...
public abstract class BarLineChartBase<T extends BarLineScatterCandleBubbleData<? extends BarLineScatterCandleBubbleDataSet<? extends Entry>>>
    ...
    protected void init() {
        ...
        mChartTouchListener = new BarLineChartTouchListener2(this, mViewPortHandler.getMatrixTouch());
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //Modify: 无数据时也画外边框 by yanmin
        drawGridBackground(canvas);

        if (mDataNotSet) {
            mXAxisRenderer.renderNodataGridLines(canvas);
            mAxisRendererLeft.renderNodataGridLines(canvas);
            mAxisRendererRight.renderNodataGridLines(canvas);
            return;
        }
        ...
        //drawGridBackground(canvas); // Modify：移动到函数开头 by yanmin
        ...
        //Bug： 当mAutoScaleMinMaxEnabled设置为true时，首次绘制Chart. 轴上的标尺值不绘制,所以需先做上面的计算然后再画（后续跟进） By yanmin
        ...
        mAxisRendererLeft.renderGridLines(canvas);//画对应的坐标值的虚线
        ...
        //Modify by yanmin: 把当前界面可视区域的LowestIndex和HighestIndex传给mXAxisRenderer
        mXAxisRenderer.setXIndexRange(mAutoScaleLastLowestVisibleXIndex, mAutoScaleLastHighestVisibleXIndex);
        mXAxisRenderer.renderAxisLabels(canvas);
        mAxisRendererLeft.renderAxisLabels(canvas);//画对应标尺线的坐标值
        mAxisRendererRight.renderAxisLabels(canvas);

        //Modify by yanmin: 把drawHighlighted移动到renderAxisLabels下面，是为了防止Highlight的Text被label覆盖
...
//            mAxisRight.setValueFormatter(mDefaultFormatter);
    ...
    protected void calcMinMax() {
        ...
        // Modify by yanmin: 使Y轴分布以中间值为界，上下平均分布，如最高显示10%，则最低就显示-10%
        if (mAxisLeft.isMeristic()) {
            float median = mAxisLeft.getMedian();
            float f1 = Math.abs(minLeft - median);
            float f2 = Math.abs(maxLeft - median);
            float larger = (f1 > f2) ? f1 : f2;

            minLeft = median - larger;
            maxLeft = median + larger;
        }

        if (mAxisRight.isMeristic()) {
            float median = mAxisRight.getMedian();
            float f1 = Math.abs(minRight - median);
            float f2 = Math.abs(maxRight - median);
            float larger = (f1 > f2) ? f1 : f2;

            minRight = median - larger;
            maxRight = median + larger;
        }
        ...
        if (mAxisLeft.isStartAtZeroEnabled()) {
            if (minLeft < 0.f && maxLeft < 0.f) {
                ...
                // Stick the minimum to 0.0 or less, and maximum to 0.0 or down (startAtZero for negative/positive at the same time)
                mAxisLeft.mAxisMinimum = Math.min(0.f, !Float.isNaN(mAxisLeft.getAxisMinValue()) ? mAxisLeft.getAxisMinValue() : (minLeft - bottomSpaceLeft));
                mAxisLeft.mAxisMaximum = Math.max(0.f, !Float.isNaN(mAxisLeft.getAxisMaxValue()) ? mAxisLeft.getAxisMaxValue() : (maxLeft + topSpaceLeft));
            }
        } else {
            // Use the values as they are
            //Modify by yanmin:解决蜡烛图中，min会成为负数的情况，价格不可能为负
            float minValue = minLeft - bottomSpaceLeft;
            if (minLeft > 0 && minValue < 0)
                minValue = 0;

            mAxisLeft.mAxisMinimum = !Float.isNaN(mAxisLeft.getAxisMinValue()) ? mAxisLeft.getAxisMinValue() : minValue;
        ...
        if (mAxisRight.isStartAtZeroEnabled()) {
            ...
            //Modify by yanmin:解决蜡烛图中，min会成为负数的情况，价格不可能为负
            float minValue = minRight - bottomSpaceRight;
            if (minRight > 0 && minValue < 0)
                minValue = 0;

            mAxisRight.mAxisMinimum = !Float.isNaN(mAxisRight.getAxisMinValue()) ? mAxisRight.getAxisMinValue() : minValue;
    ...
    protected void drawGridBackground(Canvas c) {
        ...
        if (mDrawBorders) {
            //Modify: Border修改成只画上下边界不画左右边界  by yanmin
            //RectF rectF = mViewPortHandler.getContentRect();
            //c.drawLine(rectF.left, rectF.top, rectF.right, rectF.top, mBorderPaint);
            //c.drawLine(rectF.left, rectF.bottom, rectF.right, rectF.bottom, mBorderPaint);
            //c.drawRect(mViewPortHandler.getContentRect(), mBorderPaint);

            //modify by hualei: 根据strokeWidth绘制矩形，strokeWidth根据坐标中心绘制对应线条，所以需要对超出屏幕范围内的线条做偏移操作
            RectF oriRectF = mViewPortHandler.getContentRect();
            RectF rectF = new RectF();
            int strokeWidth = (int) mBorderPaint.getStrokeWidth(); //从配置中取strokeWidth，截取整数部分进行绘制
            mBorderPaint.setStrokeWidth(strokeWidth > 0 ? strokeWidth : 1);
            float offset = (strokeWidth) / 2f;

            //modified by hualei:参考XAxisRenderer中computeAxis，计算label的高度，并减去y的偏移量，在无数据的时候也绘制好chart2的准确高度。
            if (mXAxis.isDrawLabelsEnabled()) {
                Paint paint = new Paint(Paint.ANTI_ALIAS_FLAG);
                paint.setTextSize(mXAxis.getTextSize());

                StringBuilder widthText = new StringBuilder();

                int max = Math.round(mAxisLeft.mAxisMinimum + mXAxis.getSpaceBetweenLabels());
                for (int i = 0; i < max; i++) {
                    widthText.append('h');
                }

                final FSize labelSize = Utils.calcTextSize(paint, widthText.toString());
                final float labelWidth = labelSize.width;
                final float labelHeight = Utils.calcTextHeight(paint, "Q");
                ...
                int labelRotatedHeight = Math.round(labelRotatedSize.height);

                if (mXAxis.mLabelRotatedHeight <= 1) {
                    rectF.set(oriRectF.left + offset, oriRectF.top + offset, oriRectF.right - offset, oriRectF.bottom - offset - labelRotatedHeight - mXAxis.getYOffset());
                } else {
                    rectF.set(oriRectF.left + offset, oriRectF.top + offset, oriRectF.right - offset, oriRectF.bottom - offset);
                }
            } else {
                rectF.set(oriRectF.left + offset, oriRectF.top + offset, oriRectF.right - offset, oriRectF.bottom - offset);
            }
            c.drawRect(rectF, mBorderPaint);
        }
    }
     ...
     * 10, no down than 10 values on the x-axis can be viewed at once without
     * scrolling.
     *
     ...
     * values and no down that 100 values can be viewed at once without
     * scrolling
     *
     * @param minXRange
     * @param maxXRange
    ...
    /**
     * Moves the left side of the current viewport to the specified x-index,immediately
     * This also refreshes the chart by calling invalidate().
     * Add By yanmin
     *
     * @param xIndex
     */
    public void moveViewToXSync(float xIndex) {
    ...
    /**
     * Add By yanmin
     * Returns the lowest x-index (value(float) on the x-axis) that is still visible on
     * the chart.
     *
     * @return
     */
    ...
    /**
     * Add By yanmin
     * Returns the highest x-index (value(float) on the x-axis) that is still visible
     * on the chart.
     *
     * @return
     */
