...
public class YAxis extends AxisBase {
    ...
    // Modify by yanmin
    public boolean mMeristic = false;
    public float mAxisMedian = 0f;
    ...
    // Modify by yanmin: 添加一个中间值属性，如果图表有设置中间值，则大于中间值的显示mIncreasingColor颜色，
    // 小于则显示mDecreasingColor
    /** color for increase then Median*/
    protected int mIncreasingColor = ColorTemplate.COLOR_NONE;

    /** color for decrease then Median */
    ...
    public void setMedian(float v,int increaseColor,int decreaseColor) {
        mMeristic = true;
        mAxisMedian = v;
        mIncreasingColor = increaseColor;
        mDecreasingColor = decreaseColor;
    }
    ...
    public boolean isMeristic() {
        return mMeristic;
    }

    public int isLargerThanMedian(int index) {
        if (index < 0 || index >= mEntries.length)
            return 0;
        ...
        float v = mEntries[index];
        if (v > mAxisMedian)
            return 1;
        else if (v < mAxisMedian)
            return -1;
        return 0;
    }
    //End by yanmin
