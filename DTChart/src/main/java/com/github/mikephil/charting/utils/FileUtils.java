...
import com.github.mikephil.charting.data.CandleExData;
...
public class FileUtils {
    ...
    //Add: 添加从文件读取蜡烛图数据 By yanmin
    public static List<CandleEntry> loadCandleEntriesFromFile(String path) {
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (split.length > 4) {
                    CandleExData exData = new CandleExData();
                    exData.setTime(Long.parseLong(split[0]));
                    ...
                    entries.add(new CandleEntry(i++, high, low, open, close, exData));
                }
            }
        } catch (IOException e) {
            Log.e(LOG, e.toString());
